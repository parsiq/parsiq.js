...
export async function runBlocks() {
    ...
    for await (const blockByBlockNumber of client.blocks.getByBlockRange(
        0,
        'latest',
        {limit: 1}
    )) {
    ...
    for await (const blockByTimestamp of client.blocks.getByTimestamp(
    ...
    const blockByHash = await client.blocks.getByHash('0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3');
    console.log(`Block by hash - ${JSON.stringify(blockByHash)}`, '\n')

    for await (const blockByBlockNumberWithOffset of client.blocks.getByBlockRange(
        0,
        1,
