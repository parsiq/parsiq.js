...
export class NftRequestHandler extends HttpClient {
  ...
  public async *getNftByAddress(
    address: string,
    criteria: NftContractCriteria,
    boundaries: RangeOptions,
  ): AsyncGenerator<NftAddressInventoryItem, void, undefined> {
    const iterator = this.query<NftAddressInventoryItem>(`/${address}/inventory`, criteria, boundaries);
    for await (const data of iterator) {
      yield* data;
    }
  }

  public async *getNftHistoryByAddress(
    address: string,
    criteria: NftContractCriteria,
    boundaries: RangeOptions,
  ): AsyncGenerator<NftAddressInventoryHistoryItem, void, undefined> {
    const iterator = this.query<NftAddressInventoryHistoryItem>(`/${address}/history`, criteria, boundaries);
    for await (const data of iterator) {
      yield* data;
    }
  }

  public async *getNftHistory(
    tokenId: string,
    contract: string,
    criteria: NftSupplementalDataCriteria,
    boundaries: RangeOptions,
  ): AsyncGenerator<NftCollectionTokenHolder, void, undefined> {
    const iterator = this.query<NftCollectionTokenHolder>(`/${contract}/${tokenId}/history`, criteria, boundaries);
    for await (const data of iterator) {
      yield* data;
    }
  }
  ...
  public async *getCollectionHolders(
    ...
    criteria: NftSupplementalDataCriteria,
    boundaries: RangeOptions,
  ): AsyncGenerator<NftTokenTransferItem, void, undefined> {
    const iterator = this.query<NftTokenTransferItem>(`/${contract}/owner`, criteria, boundaries);
    for await (const data of iterator) {
      yield* data;
    }
  }
